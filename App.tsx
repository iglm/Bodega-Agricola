...
import { Package, Pickaxe, Target, Tractor, Database, Settings, Globe, ChevronDown, Download, Plus, TrendingUp, HelpCircle, Calendar, Zap, CalendarRange, Loader2, AlertTriangle, ShieldCheck, Sprout, Calculator, Lightbulb, History as HistoryIcon } from 'lucide-react';
...
function App() {
  ...
  // ESTADO LOCAL-FIRST ESTRICTO
  const [data, setData] = useState<AppState>(() => {
      try {
          return loadDataFromLocalStorage();
      } catch (error) {
          console.error("Error cargando locales, iniciando vacío:", error);
          // Si falla, retornamos un estado inicial vacío, NO los datos demo por defecto.
          return {
  ...
  const handleDeleteItem = (id: string) => {
    setData(prev => ({ ...prev, inventory: prev.inventory.filter(i => i.id !== id), movements: prev.movements.filter(m => m.itemId !== id) }));
    setDeleteItem(null);
  };

  const handleUpdateSWOT = (swot: SWOT) => { setData(prev => ({ ...prev, swot })); };
  const handleToggleBpa = (code: string) => { setData(prev => ({ ...prev, bpaChecklist: { ...prev.bpaChecklist, [code]: !prev.bpaChecklist[code] } })); };
  const handleAddAsset = (asset: Omit<Asset, 'id' | 'warehouseId'>) => { setData(prev => ({ ...prev, assets: [...(prev.assets || []), { ...asset, id: generateId(), warehouseId: activeId }] })); };
  const handleUpdateMachine = (machine: Machine) => { setData(prev => ({ ...prev, machines: prev.machines.map(m => m.id === machine.id ? machine : m) })); };
  const handleDeleteAsset = (id: string) => { setData(prev => ({ ...prev, assets: prev.assets.filter(a => a.id !== id) })); };

  const handleLoadDemoData = () => {
    const demoData = getDemoData();
    setData(demoData);
    setSession({ id: 'user_demo_datosfinca', name: 'Usuario Demo', email: 'demo@datosfinca.com', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=DatosFincaVivaDemo' });
    setView('app');
  ...
  const handleAddPlannedLabor = (labor: Omit<PlannedLabor, 'id' | 'warehouseId' | 'completed'>) => {
      setData(prev => ({ ...prev, plannedLabors: [...(prev.plannedLabors || []), { ...labor, id: generateId(), warehouseId: activeId, completed: false }] }));
      showNotification('Labor programada exitosamente', 'success');
  };

  const handleDeletePlannedLabor = (id: string) => { setData(prev => ({ ...prev, plannedLabors: prev.plannedLabors.filter(l => l.id !== id) })); };
  const handleTogglePlannedLabor = (id: string) => { setData(prev => ({ ...prev, plannedLabors: prev.plannedLabors.map(l => l.id === id ? { ...l, completed: !l.completed } : l) })); };
  const handleUpdateCostCenter = (updatedLot: CostCenter) => { setData(prev => ({ ...prev, costCenters: prev.costCenters.map(c => c.id === updatedLot.id ? updatedLot : c) })); showNotification('Lote actualizado exitosamente', 'success'); };

  const handleSaveBudget = (budget: BudgetPlan) => {
      const exists = data.budgets?.find(b => b.id === budget.id);
      let newBudgets = data.budgets || [];
      if (exists) { newBudgets = newBudgets.map(b => b.id === budget.id ? budget : b); } 
      else { newBudgets = [...newBudgets, budget]; }
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      {view === 'landing' && (
        <Landing onEnter={(u) => { setSession(u); setView('app'); }} onShowManual={() => setShowManual(true)} onRestoreBackup={() => requestSecureAction(() => setShowData(true))} onLoadDemoData={handleLoadDemoData} />
      )}
      ...
      {view === 'app' && data && (
        <>
          ...
          <main className="max-w-4xl mx-auto p-4 pb-40">
            ...
            {currentTab === 'simulator' && <SimulatorView laborLogs={data.laborLogs.filter(l=>l.warehouseId === activeId)} harvests={data.harvests.filter(h=>h.warehouseId === activeId)} inventory={data.inventory.filter(i=>i.warehouseId === activeId)} />} 

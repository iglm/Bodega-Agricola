...
import { Settings, Wrench, Droplets, Plus, Trash2, Fuel, PenTool, FileText, FileSpreadsheet, Download, Gauge, User, MapPin, Pickaxe, DollarSign, CheckCircle, ArrowRight, Tractor, Microscope, ShieldCheck, Recycle, Signature, UserCheck, ShieldAlert, FileCheck, Pencil, Globe, ClipboardList, Briefcase, Droplet, AlertTriangle, Bookmark, Shield, Zap, Info, Clock, CheckCircle2, Leaf, Bug, FlaskConical, Scale, Warehouse, HardHat, ChevronDown, ChevronUp, AlertCircle, Award, Sprout, Coffee, Gavel, Lock, Fingerprint, Copyright } from 'lucide-react';
...
// ... (BPA Templates keep original data) ...
...
export const ManagementView: React.FC<ManagementViewProps> = ({
  ...
  const bpaSummary = useMemo(() => {
    ...
    if (auditStandard === 'ICA') {
        ...
        criteria.forEach(c => { if (!grouped[c.category]) grouped[c.category] = []; grouped[c.category].push(c); });
        return { criteria, percent: (compliant / criteria.length) * 100, criticalFail, grouped, majorPercent: 0, minorPercent: 0 };
    } else {
        ...
        const grouped: Record<string, BpaCriterion[]> = {};
        criteria.forEach(c => { if (!grouped[c.category]) grouped[c.category] = []; grouped[c.category].push(c); });
        return { criteria, percent: (majorPercent + minorPercent) / 2, majorPercent, minorPercent, criticalFail, grouped };
    }
  }, [bpaChecklist, auditStandard]);

  const toolCalculations = useMemo(() => {
      let dischargeLmin = parseFloat(toolDischarge);
      if (toolDischargeUnit === 'ml' || toolDischargeUnit === 'cc') dischargeLmin = dischargeLmin / 1000;
      ...
      if (LHa > 0 && tank > 0 && dose > 0) productPerTank = (tank / LHa) * dose;
  ...
  return (
    <div className="space-y-6 pb-20 animate-fade-in">
        <div className="flex bg-slate-200 dark:bg-slate-800 p-1 rounded-xl overflow-x-auto scrollbar-hide gap-1 sticky top-[120px] z-30 shadow-md">
            ...
            <button onClick={() => setSubTab('integrity')} className={`shrink-0 px-4 py-2 rounded-lg text-[10px] font-black uppercase flex items-center gap-2 transition-all ${subTab === 'integrity' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500'}`}><Fingerprint className="w-3 h-3" /> Blindaje</button>
        ...
        {subTab === 'audit' && (
            <div className="space-y-6 animate-fade-in">
                <div className="flex flex-wrap gap-2 justify-center bg-slate-900 p-2 rounded-2xl relative z-0">
                    <button onClick={() => { setAuditStandard('ICA'); setExpandedCategory(null); }} className={`flex-1 py-3 px-4 min-w-[120px] rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 transition-all relative z-10 ${auditStandard === 'ICA' ? 'bg-white text-slate-900 shadow-lg' : 'text-slate-500 hover:text-white'}`}><ShieldCheck className="w-4 h-4" /> Res. ICA</button>
                    <button onClick={() => { setAuditStandard('GLOBALGAP'); setExpandedCategory(null); }} className={`flex-1 py-3 px-4 min-w-[120px] rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 transition-all relative z-10 ${auditStandard === 'GLOBALGAP' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}><Globe className="w-4 h-4" /> GLOBALG.A.P.</button>
                    <button onClick={() => { setAuditStandard('CODE_4C'); setExpandedCategory(null); }} className={`flex-1 py-3 px-4 min-w-[120px] rounded-xl text-xs font-black uppercase flex items-center justify-center gap-2 transition-all relative z-10 ${auditStandard === 'CODE_4C' ? 'bg-orange-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}><Coffee className="w-4 h-4" /> Código 4C</button>
                </div>
                <div className={`p-8 rounded-[3rem] border-4 transition-all relative overflow-hidden ${bpaSummary.criticalFail ? 'bg-slate-900 border-red-500/30' : 'bg-slate-900 border-emerald-500/30'}`}>
                    <div className="relative z-10 flex flex-col items-center">
                        <h3 className="font-black text-2xl flex items-center justify-center gap-3 uppercase tracking-tighter mb-6 text-white text-center leading-tight">{auditStandard === 'ICA' ? <ShieldCheck className="w-8 h-8 text-white" /> : auditStandard === 'GLOBALGAP' ? <Globe className="w-8 h-8 text-emerald-400" /> : <Coffee className="w-8 h-8 text-orange-400" />}{auditStandard === 'ICA' ? 'Certificación ICA' : auditStandard === 'GLOBALGAP' ? 'GLOBALG.A.P. IFA' : 'Código 4C v4.1'}</h3>
                        {auditStandard === 'ICA' ? (
                            <div className="relative w-48 h-48 mx-auto mb-6">
                                <svg className="w-full h-full transform -rotate-90 drop-shadow-2xl"><circle cx="96" cy="96" r="88" stroke="currentColor" strokeWidth="12" fill="transparent" className="text-slate-800" /><circle cx="96" cy="96" r="88" stroke="currentColor" strokeWidth="12" fill="transparent" strokeDasharray={552} strokeDashoffset={552 - (552 * bpaSummary.percent / 100)} className={`transition-all duration-1000 ${bpaSummary.criticalFail ? 'text-red-500' : 'text-emerald-500'}`} strokeLinecap="round" /></svg>
                                <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-5xl font-black text-white">{bpaSummary.percent.toFixed(0)}%</span><span className="text-[10px] text-slate-400 font-bold uppercase mt-1">Cumplimiento</span></div>
                            </div>
                        ) : (
                            <div className="w-full space-y-6 mb-6">
                                <div><div className="flex justify-between items-end mb-2"><span className="text-xs font-black text-red-400 uppercase flex items-center gap-2"><Award className="w-4 h-4"/> Obligaciones Mayores</span><span className={`text-lg font-black font-mono ${bpaSummary.majorPercent < 100 ? 'text-red-500' : 'text-emerald-500'}`}>{bpaSummary.majorPercent.toFixed(0)}% <span className="text-[9px] text-slate-500">/ 100%</span></span></div><div className="w-full h-4 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${bpaSummary.majorPercent < 100 ? 'bg-red-500' : 'bg-emerald-500'}`} style={{width: `${bpaSummary.majorPercent}%`}}></div></div></div>
                                <div><div className="flex justify-between items-end mb-2"><span className="text-xs font-black text-blue-400 uppercase flex items-center gap-2"><CheckCircle className="w-4 h-4"/> Mejora Continua</span><span className={`text-lg font-black font-mono ${bpaSummary.minorPercent < 95 ? 'text-amber-500' : 'text-emerald-500'}`}>{bpaSummary.minorPercent.toFixed(0)}% <span className="text-[9px] text-slate-500">/ 95%</span></span></div><div className="w-full h-4 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${bpaSummary.minorPercent < 95 ? 'bg-amber-500' : 'bg-blue-500'}`} style={{width: `${bpaSummary.minorPercent}%`}}></div></div></div>
                            </div>
                        )}
                        {bpaSummary.criticalFail ? (<div className="bg-red-500/20 border border-red-500 text-red-200 px-6 py-3 rounded-2xl flex items-center gap-3 animate-pulse"><AlertTriangle className="w-6 h-6 text-red-500" /><div className="text-left"><p className="font-black text-xs uppercase">No Certificable</p><p className="text-[10px]">Fallan Puntos Críticos / Umbrales</p></div></div>) : (<div className="bg-emerald-500/20 border border-emerald-500 text-emerald-200 px-6 py-3 rounded-2xl flex items-center gap-3"><CheckCircle className="w-6 h-6 text-emerald-500" /><div className="text-left"><p className="font-black text-xs uppercase">Listo para Auditoría</p><p className="text-[10px]">Umbrales de cumplimiento OK</p></div></div>)}
                        <p className="text-[9px] text-slate-500 mt-4 uppercase font-bold tracking-widest text-center">{auditStandard === 'ICA' ? 'Auditoría Oficial Res. ICA 082394 / 30021' : auditStandard === 'GLOBALGAP' ? 'Basado en GLOBALG.A.P. IFA v6 GFS/Smart' : 'Código de Conducta 4C v4.1 (2025)'}</p>
                    </div>
                </div>
                <div className="space-y-4">
                    {Object.entries(bpaSummary.grouped).map(([catKey, items]) => {
                        ...
                        const progress = (compliantCount / typedItems.length) * 100;
                        ...
                        return (
                            <div key={catKey} className={`rounded-[2rem] border overflow-hidden transition-all duration-300 ${isExpanded ? 'bg-slate-900 border-slate-700 shadow-xl' : 'bg-slate-900/50 border-slate-800'}`}>
                                <button onClick={() => setExpandedCategory(isExpanded ? null : catKey)} className="w-full p-5 flex items-center justify-between"><div className="flex items-center gap-4"><div className={`p-3 rounded-xl ${isExpanded ? 'bg-slate-800' : 'bg-slate-800/50'}`}><Icon className={`w-6 h-6 ${isExpanded ? 'text-indigo-400' : 'text-slate-500'}`} /></div><div className="text-left"><h4 className={`text-sm font-black uppercase tracking-wide text-white`}>{catKey}</h4><div className="flex items-center gap-2 mt-1"><div className="w-20 h-1.5 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full ${hasCriticalFail ? 'bg-red-500' : 'bg-emerald-500'}`} style={{width: `${progress}%`}}></div></div><span className="text-[10px] font-mono text-slate-400">{compliantCount}/{typedItems.length}</span></div></div></div><div className="flex items-center gap-3">{hasCriticalFail && !isExpanded && (<div className="bg-red-500/20 p-2 rounded-full animate-pulse"><AlertCircle className="w-4 h-4 text-red-500" /></div>)}{isExpanded ? <ChevronUp className="w-5 h-5 text-slate-500" /> : <ChevronDown className="w-5 h-5 text-slate-500" />}</div></button>
                                {isExpanded && (<div className="p-5 pt-0 border-t border-slate-800/50 space-y-3 animate-fade-in-down"><div className="h-4"></div> {typedItems.map(c => (<div key={c.code} onClick={() => onToggleBpa(c.code)} className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-start gap-4 group ${c.compliant ? 'bg-emerald-950/20 border-emerald-500/20' : 'bg-slate-950 border-slate-800 hover:border-slate-600'}`}><div className={`mt-0.5 p-1 rounded-full shrink-0 border ${c.compliant ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-transparent border-slate-600 text-transparent'}`}><CheckCircle2 className="w-3 h-3" /></div><div className="flex-1"><div className="flex items-center gap-2 mb-1"><span className="text-[9px] font-black text-slate-500">{c.code}</span>{c.complianceLevel === 'MAJOR' ? (<span className="text-[8px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded border border-red-500/30 uppercase font-black tracking-wider">Mayor</span>) : (<span className="text-[8px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded border border-blue-500/30 uppercase font-black tracking-wider">Menor</span>)}</div><p className={`text-xs font-bold leading-snug ${c.compliant ? 'text-emerald-100 line-through decoration-emerald-500/50' : 'text-slate-300'}`}>{c.label}</p></div></div>))}</div>)}
                            </div>
                        );
                    })}
                </div>
        ...
        {/* --- NUEVO MÓDULO: BLINDAJE LEGAL Y ANTIPIRATERÍA --- */}
        {subTab === 'integrity' && (
            <div className="space-y-6 animate-fade-in">
                <div className="bg-slate-900 p-8 rounded-[3rem] border border-slate-700 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative z-10 space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-white font-black text-xl uppercase italic">Integridad y Propiedad Intelectual</h3>
                        </div>
                        <p className="text-xs text-slate-400 leading-relaxed font-medium">
                            Este software es propiedad intelectual de <strong>Lucas Mateo Tabares Franco</strong>. La ingeniería inversa, redistribución no autorizada o modificación del núcleo de base de datos local está prohibida y protegida por la ley colombiana.
                        </p>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                                <h4 className="text-[10px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-2"><Gavel className="w-4 h-4"/> Marco Legal Citado</h4>
                                <ul className="space-y-1.5">
                                    <li className="text-[9px] text-slate-500 font-bold flex items-center gap-2"><div className="w-1 h-1 bg-indigo-500 rounded-full"/> Ley 23 de 1982 (Derechos de Autor)</li>
                                    <li className="text-[9px] text-slate-500 font-bold flex items-center gap-2"><div className="w-1 h-1 bg-indigo-500 rounded-full"/> Ley 1581 de 2012 (Habeas Data)</li>
                                    <li className="text-[9px] text-slate-500 font-bold flex items-center gap-2"><div className="w-1 h-1 bg-indigo-500 rounded-full"/> Ley 1915 de 2018 (Copyright Digital)</li>
                                </ul>
                            </div>
                            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                                <h4 className="text-[10px] font-black text-indigo-500 uppercase tracking-widest flex items-center gap-2"><ShieldAlert className="w-4 h-4"/> Anti-Piratería</h4>
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center bg-slate-900 p-2 rounded-lg border border-slate-800">
                                        <span className="text-[8px] text-slate-500 font-black">ESTADO LICENCIA:</span>
                                        <span className="text-[8px] text-emerald-500 font-black bg-emerald-500/10 px-2 py-0.5 rounded">ORIGINAL V1.0</span>
                                    </div>
                                    <div className="flex justify-between items-center bg-slate-900 p-2 rounded-lg border border-slate-800">
                                        <span className="text-[8px] text-slate-500 font-black">FIRMA DIGITAL:</span>
                                        <span className="text-[8px] text-indigo-400 font-black font-mono">LM-TF-2025-VIVA</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        
                        <div className="p-4 bg-indigo-900/10 rounded-2xl border border-indigo-500/20 text-center">
                            <p className="text-[9px] text-indigo-300 font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                                <Copyright className="w-3 h-3" /> © 2025 DatosFinca Viva | Lucas Mateo Tabares Franco
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="space-y-6 animate-fade-in">
                 <div className="bg-slate-900/50 p-6 rounded-[2rem] border border-slate-700 space-y-4">
                    ...
                    <div className="bg-slate-900 p-4 rounded-2xl border border-slate-700 relative">
                        ...
                        <h5 className="text-[10px] font-black uppercase text-slate-400 mb-3 flex items-center gap-2"><Gauge className="w-3 h-3" /> Calibración del Equipo</h5>
                         <div className="space-y-3">
                            <div>
                                ...
                                <div className="flex">
                                    ...
                                    <select value={toolDischargeUnit} onChange={e => setToolDischargeUnit(e.target.value as any)} className="bg-slate-800 border border-slate-600 rounded-r-xl px-2 text-xs text-amber-500 font-bold outline-none"><option value="L">L/min</option><option value="ml">ml/min</option><option value="cc">cc/min</option></select>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div><label className="text-[9px] text-slate-500 font-bold uppercase ml-1">Ancho Faja (m)</label><input value={toolWidth} onChange={e=>setToolWidth(e.target.value)} placeholder="Metros" className="w-full bg-slate-800 border border-slate-600 rounded-xl p-3 text-sm text-white font-mono" /></div>
                                <div><label className="text-[9px] text-slate-500 font-bold uppercase ml-1">Velocidad (Km/h)</label><input value={toolSpeed} onChange={e=>setToolSpeed(e.target.value)} placeholder="Km/h" className="w-full bg-slate-800 border border-slate-600 rounded-xl p-3 text-sm text-white font-mono" /></div>
                            </div>
                         </div>
                         <div className="mt-4 p-4 rounded-2xl bg-amber-950/50 border border-amber-500/20 text-center flex items-center justify-between"><div className="text-left"><p className="text-[10px] uppercase font-black text-amber-500">Gasto de Agua</p><p className="text-[9px] text-slate-500">Volumen de aplicación</p></div><p className="text-3xl font-mono font-black text-white">{toolCalculations.LHa.toFixed(1)} <span className="text-xs text-amber-500">L/Ha</span></p></div>
                    </div>
                    <div className="bg-slate-900 p-4 rounded-2xl border border-slate-700 relative">
                        ...
                        <h5 className="text-[10px] font-black uppercase text-slate-400 mb-3 flex items-center gap-2"><FlaskConical className="w-3 h-3" /> Preparación de Mezcla</h5>
                         <div className="grid grid-cols-2 gap-3">
                            <div><label className="text-[9px] text-slate-500 font-bold uppercase ml-1">Tamaño Tanque</label><div className="flex items-center bg-slate-800 border border-slate-600 rounded-xl px-3"><input value={toolTank} onChange={e=>setToolTank(e.target.value)} placeholder="0" className="w-full bg-transparent border-none p-3 pl-0 text-sm text-white font-mono outline-none" /><span className="text-xs text-slate-500 font-bold">Lts</span></div></div>
                            <div><label className="text-[9px] text-slate-500 font-bold uppercase ml-1">Dosis Recomendada</label><div className="flex"><input value={toolDose} onChange={e=>setToolDose(e.target.value)} placeholder="0.0" className="flex-1 bg-slate-800 border-y border-l border-slate-600 rounded-l-xl p-3 text-sm text-white font-mono w-full min-w-0" /><select value={toolDoseUnit} onChange={e => setToolDoseUnit(e.target.value as any)} className="bg-slate-800 border border-slate-600 rounded-r-xl px-1 text-[10px] text-emerald-500 font-bold outline-none max-w-[60px]"><option value="cc">cc/Ha</option><option value="ml">ml/Ha</option><option value="L">L/Ha</option><option value="g">g/Ha</option><option value="Kg">Kg/Ha</option></select></div></div>
                         </div>
                          <div className="mt-4 p-4 rounded-2xl bg-emerald-950/50 border border-emerald-500/20 text-center"><p className="text-[10px] uppercase font-black text-emerald-500 mb-1">Producto a agregar al Tanque</p><p className="text-3xl font-mono font-black text-white">{toolCalculations.productPerTank.toFixed(1)} <span className="text-lg text-emerald-400">{toolDoseUnit}</span></p><p className="text-[9px] text-slate-500 mt-1 italic">Calculado automáticamente según el gasto de agua.</p></div>
                    </div>
                 </div>
            </div>
        )}
